...
if ($ITBoostData.ContainsKey("$sourceProperty")){
    ...
    foreach ($listField in $ListFields){
        $ListMaps["$($listField.label)"]=$(get-hudulists -id $listfield.id)
    }
    ...
    foreach ($company in $groupedflexis.Keys) {
        ...
        foreach ($companyflexi in $flexisForCompany){
            ...
            foreach ($key in $flexisMap.Keys | where-object {$_ -ne "location" -and $smooshLabels -notcontains $_ -and $PasswordsFields -notcontains $_}) {
                ...
                $MatchedList =$null; $MatchedList = $(if ($ListMaps.ContainsKey("$huduField")) {$ListMaps["$huduField"]} else {$null})
                if ($null -ne $MatchedList) {
                    $Listoptions = $MatchedList.list_items
                    write-host "Comparing row val $($rowval) to available List options $($Listoptions -join ", ")" -ForegroundColor Green
                    $bestChoice=$null; $bestChoice  = ChoseBest-ByName -choices $Listoptions -name $rowVal;
                    if (-not $bestChoice -or [string]::IsNullOrWhiteSpace($bestChoice)){
                        if ($false -eq $createNewItemsForLists) {continue}
                        write-host "Adding new list option: $rowval to listitems for $($hudulist.Name)"
                        $NewOptions = $Listoptions
                        $NewOptions += $rowVal
                        set-hudulist -id $huduList.id -name $HuduList.Name -listItems $NewOptions
                        $ListMaps["$huduField"] = get-hudulists -id $MatchedList.id
                        write-host "Updated list cache with new item"
                    } else {$rowVal = $bestChoice}
