...
if (-not $contactsLayout){
    ...
    @{label="Office Phone number (DID)";   show_in_list=$false;   field_type="Phone";required=$false;   hint="";   position=4},
...
$groupedContacts = $ITBoostData.contacts.CSVData | Group-ObjectSafeHashTable { $_.organization }
try {
    $allHuduContacts = Get-HuduAssets -AssetLayoutId $contactsLayout.id
} catch {
    $allHuduContacts=@()
